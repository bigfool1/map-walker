...
func newStatsTestServer(t *testing.T) *Server {
	t.Helper()
	srv := newTestServer(t)
	srv.WithAdmin(stubHubSnapshot, stubSyntheticSnapshot)
	return srv
}
...
func TestStatsReturns405ForNonGet(t *testing.T) {
	srv := newStatsTestServer(t)
	server := httptest.NewServer(srv.Routes())
	t.Cleanup(server.Close)

	req, _ := http.NewRequest(http.MethodPost, server.URL+"/api/stats/synthetic", nil)
...
// --- /api/stats/synthetic success ---

func TestStatsReturns200(t *testing.T) {
	srv := newStatsTestServer(t)
	server := httptest.NewServer(srv.Routes())
	t.Cleanup(server.Close)

	resp, err := http.Get(server.URL + "/api/stats/synthetic")
	if err != nil {
		t.Fatalf("request: %v", err)
	}
	defer resp.Body.Close()
...
func TestStatsResponseContainsAggregateFields(t *testing.T) {
	srv := newStatsTestServer(t)
	server := httptest.NewServer(srv.Routes())
	t.Cleanup(server.Close)
	...
	if body.Synthetic.TargetCount != 5 {
		t.Errorf("synthetic.TargetCount=%d want 5", body.Synthetic.TargetCount)
	}
...
func TestStatsResponseContainsNoClientIdentities(t *testing.T) {
	srv := newStatsTestServer(t)
	server := httptest.NewServer(srv.Routes())
	t.Cleanup(server.Close)
...
func TestStatsNilSnapshotsWhenNoManager(t *testing.T) {
	srv := newTestServer(t)
	// Configured with token but no synthetic provider.
	srv.WithAdmin(stubHubSnapshot, nil)
	server := httptest.NewServer(srv.Routes())
	t.Cleanup(server.Close)
